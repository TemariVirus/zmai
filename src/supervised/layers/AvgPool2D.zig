...
//! slices. This layer has no trainable parameters. This layer works with
//! overlapping pools.
...
pub fn forward(self: Self, input: []const f32, output: []f32) void {
    ...
    const pool_area: f32 = @floatFromInt(self.pool_size.x * self.pool_size.y);
    ...
    for (0..self.input_shape.z) |z| {
        ...
        while (y + self.pool_size.y <= self.input_shape.y) : (y += self.stride.y) {
            ...
            while (x + self.pool_size.x <= self.input_shape.x) : (x += self.stride.x) {
                // Take the average
                output[out_i] = self.sumPool(x, y, z, input) / pool_area;
                out_i += 1;
            }
        }
    }
...
inline fn sumPool(self: Self, x: usize, y: usize, z: usize, input: []const f32) f32 {
    var sum: f32 = 0.0;
